...
How it is used:
Let's return to school and remember our basic math principles. Sometimes simple arithmetic allows us to resolve difficult paradox problems easily.
...
def chase(a1_speed, t2_speed, advantage):
    # 'v1'과 'v2'는 a1과 t2의 속도로, 각각 'x'는 장점, 두 로봇의 시작과 장소 사이의 거리',
    # 'T'는 시간 이 둘 사이.
    #  T는 다음과 같이 두 로봇의 관점에서 표현 될 수 있습니다.
    # -> T = s / v1 + x
    # -> T = s / v2.
    # -> s / v1 + x = s / v2,
    # -> v1 * v2 * x / (v1 - v2) = s
